       ...
       it "works for hexcode2" $
         do let decompiled@((loc, _):_) = decompileHexString hexcode2
                result =
                  ...
                  do contract <- evmOps2HplContract decompiled
                     show <$> doCfgAugWithTopNPass contract
            result `shouldBe` "HplContract {ctorOf = HplCode {entryOf = Just L1, bodyOf = LM (UM (fromList [(1,CO: L1\n" ++
                  "OO: 0: PUSH [96]\n" ++
                  "OO: 2: PUSH [64]\n" ++
                  "OO: 4: MSTORE\n" ++
                  "OO: 5: CALLDATASIZE\n" ++
                  ...
                  "OO: 213: JUMPDEST\n" ++
                  "OC: 214: STOP -> []\n" ++
                  ")]))}, dispatcherOf = HplCode {entryOf = Nothing, bodyOf = LM (UM (fromList []))}}"
