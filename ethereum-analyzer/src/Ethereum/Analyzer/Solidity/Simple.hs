...
newtype Idfr = Idfr Text
  deriving (Eq, Generic, Show, GP.Out)

data LValue = JustId Idfr
  | Index { iArray :: LValue
          , iIndex :: LValue }
  | Member { mObj :: LValue
           , mField :: Idfr }
...
s2sStatements SolNode { name = Just "Assignment"
  ...
  let (prelval, simpleLval) = s2sLval lval
      (prerval, simpleRval) = s2sLval rval in
    prerval <> prelval <> [StAssign simpleLval $ ExpLval simpleRval]
s2sStatements e@SolNode { name = Just "Return"
  ...
  in
    prerval <> [StReturn simpleRvals]
s2sStatements SolNode {name = Just "IfStatement", children = Just [cond, thenBr]} =
  let (precond, lvalcond) = s2sLval cond
      thenSts = s2sStatements thenBr in
    precond <> [StIf lvalcond thenSts []]
s2sStatements SolNode {name = Just "IfStatement", children = Just [cond, thenBr, elseBr]} =
  let (precond, lvalcond) = s2sLval cond
      thenSts = s2sStatements thenBr
      elseSts = s2sStatements elseBr in
    precond <> [StIf lvalcond thenSts elseSts]
s2sStatements SolNode { name = Just "VariableDeclarationStatement"
                      , children = Just [vdec@SolNode { name = Just "VariableDeclaration" },
                                         vinit] } =
  let varDecl@(VarDecl vId _):_ =
        s2sVarDecls vdec -- because VariableDeclaration only emits one decl
      (preDef, lvalDef) = s2sLval vinit in
    preDef <> [StLocalVarDecl varDecl] <> [StAssign (JustId vId) (ExpLval lvalDef)]
s2sStatements n@SolNode { name = Just "FunctionCall" } =
  let (precall, lvalcall) = s2sLval n in
    precall <> [StAssign (JustId $ Idfr "_") (ExpLval lvalcall)]
...
s2sLval SolNode { name = Just "MemberAccess"
  ...
  let (prelval, simpleLval) = s2sLval obj in
    (prelval, Member simpleLval (Idfr mName))
s2sLval n@SolNode { name = Just "IndexAccess"
                , children = Just (c1:ctail) } =
  let (prelval, simpleLval) = s2sLval c1
      (presub, simpleLvalSub) = handleSubscription simpleLval ctail in
    (presub <> prelval, Index simpleLval simpleLvalSub)
  where handleSubscription :: LValue -> [SolNode] -> ([Statement], LValue)
        handleSubscription lv [] = unexpectedPanic lv
        handleSubscription lv [subNode] =
          let (presub', simpleLvalSub') = s2sLval subNode in
            (presub', Index lv simpleLvalSub')
        handleSubscription lv (subNode:t) =
          let (presub', simpleLvalSub') = handleSubscription lv [subNode]
              (presub'', simpleLvalSub'') = handleSubscription simpleLvalSub' t in
            (presub'' <> presub', simpleLvalSub'')
s2sLval SolNode { name = Just "BinaryOperation"
  ...
  let (preOp1, lvalOp1) = s2sLval op1
      (preOp2, lvalOp2) = s2sLval op2 in
    (preOp1 <> preOp2 <> [StAssign (JustId $ Idfr "TodoTmp") (ExpBin vOp lvalOp1 lvalOp2)],
     JustId $ Idfr "TodoTmp")
s2sLval SolNode { name = Just "FunctionCall"
  ...
  let (preFun, lvalFun) = s2sLval func
      ...
      lvalArgs = map snd preAndlvals in
    (preArgs <> preFun <> [StAssign (JustId $ Idfr "TodoTmp") (ExpCall lvalFun lvalArgs)],
     JustId $ Idfr "TodoTmp")
s2sLval SolNode { name = Just "Literal"
                , attributes = Just SolNode { value = Just vValue }} =
  ([StAssign (JustId $ Idfr "TodotTmp") (ExpLiteral vValue)], (JustId $ Idfr "TodoTmp"))
