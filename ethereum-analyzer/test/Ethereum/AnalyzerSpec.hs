...
spec =
  describe "disasm" $ do
    it "works for hexstring1" $ do
      let disasmd1 = showT $ showOps $ disasm hexstring1
      DT.take 60 disasmd1 `shouldBe`
        "[\"0: PUSH [96]\",\"2: PUSH [64]\",\"4: MSTORE\",\"5: PUSH [2]\",\"7:"
      DT.takeEnd 60 disasmd1 `shouldBe`
        "]\",\"6989: JUMP\",\"6990: JUMPDEST\",\"6991: SWAP1\",\"6992: JUMP\"]"
    it "works for hexstring2" $ do
      let disasmd2 = showT $ showOps $ disasm hexstring2
      disasmd2 `shouldBe` DT.concat ["[\"0: PUSH [96]\",",
        "\"2: PUSH [64]\",",
        "\"4: MSTORE\",",
        "\"5: CALLDATASIZE\",",
        "\"6: ISZERO\",",
